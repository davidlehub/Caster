...
def disableR(targetRules_className,aExceptR=[]):
	for targetRule_className in targetRules_className:
		...
		try:
			...
			print "\n", "ici20200319191945| rule remove from 'gl.RbeenExclusive':", targetRule_className, " || In:",stack()[0][3],"%s|%d " % (getframeinfo(currentframe()).filename, getframeinfo(currentframe()).lineno),"| Caller:",stack()[1][3],"%s:%d" % (getframeinfo(stack()[1][0]).filename, getframeinfo(stack()[1][0]).lineno)
