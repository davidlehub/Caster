...
class VSCodeCcrRule(MergeRule):
	...
	def enter_context(self):
		print "\n", "20200314010225|enter_context call back:", " || In:",stack()[0][3],"%s|%d " % (getframeinfo(currentframe()).filename, getframeinfo(currentframe()).lineno),"| Caller:",stack()[1][3],"%s:%d" % (getframeinfo(stack()[1][0]).filename, getframeinfo(stack()[1][0]).lineno)
	# def _process_begin(self, executable, title, handle):
	def _process_begin(self): #id 20191027215124

		print "\n", "20200314010226|_process_begin call back:", " || In:",stack()[0][3],"%s|%d " % (getframeinfo(currentframe()).filename, getframeinfo(currentframe()).lineno),"| Caller:",stack()[1][3],"%s:%d" % (getframeinfo(stack()[1][0]).filename, getframeinfo(stack()[1][0]).lineno)
def get_rule():
